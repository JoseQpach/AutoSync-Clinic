...
import { Clock, Save, ArrowLeft, Mic, Paperclip, History, Zap, Loader2, Plus } from 'lucide-react';
...
export default function PsicologiaPage() {
  ...
  const [isResourceModalOpen, setIsResourceModalOpen] = useState(false); // ESTADO PARA ABRIR EL MODAL
  
  const params = useParams();
  const pacienteId = params.id as string; 

  const handleSummarize = () => {
    if (notas.length < 50) { alert('Escribe al menos 50 caracteres para un buen resumen.'); return; }
    setIsSummarizing(true);
    setTimeout(() => {
        const resumenSimulado = `[RESUMEN IA] El paciente se centró en la gestión del estrés laboral y la mejora de la higiene del sueño. Se identificaron disparadores de ansiedad.`;
    ...
    <div className="min-h-screen bg-background text-textPrimary font-sans">
      ...
      <header className="h-16 border-b border-border bg-surface flex items-center justify-between px-6 sticky top-0 z-10">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="font-bold text-white text-lg">Sesión de {pacienteMock.tipo} | ID: {pacienteId}</h1>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 p-8 overflow-auto">
          <div className="max-w-3xl mx-auto space-y-6">
            ...
            <h3 className="text-sm font-bold text-textSecondary uppercase tracking-wider pt-2">Diagnóstico Actual (DX)</h3>
            <div className="h-12 bg-background border border-border rounded-lg flex items-center p-4 text-textSecondary">
                F41.9 Trastorno de Ansiedad (Simulado)
            </div>

            <div className="space-y-2">
                <label className="text-sm font-bold text-textSecondary uppercase tracking-wider">Notas de Evolución (SOAP)</label>
                <textarea 
                    ...
                    placeholder="S (Subjetivo), O (Objetivo), A (Análisis), P (Plan)..."
                    value={notas}
            ...
            {summary && (
                <div className="bg-primary/10 border border-primary/50 rounded-xl p-4 mt-6">
                    ...
                    <p className="text-xs text-textSecondary whitespace-pre-line">{summary}</p>
                </div>
            )}

            {/* BOTÓN ADJUNTAR TAREA - CONEXIÓN GARANTIZADA */}
            <div className="flex gap-4 pt-4">
                <button 
                    onClick={() => setIsResourceModalOpen(true)} // <-- ¡ESTE ES EL CLICK CRUCIAL!
                    className="flex items-center gap-2 text-sm text-textSecondary hover:text-white px-4 py-2 border border-border rounded-lg hover:bg-surface transition-all"
                >
                    <Paperclip size={16} />
                    Adjuntar Tarea / PDF
                </button>
        ...
        <div className="w-80 border-l border-border bg-surface/50 p-4 hidden xl:block overflow-auto">
            <div className="flex items-center gap-2 mb-6 text-textSecondary">
                <History size={18} />
                <h3 className="font-bold text-sm">Historial de {pacienteId}</h3>
            </div>
        </div>
      </div>
      
      {/* MODAL DE ASIGNACIÓN (DEBE ABRIRSE AQUÍ) */}
      <AssignResourceModal 
